...
import string
import re
from tile_library.constants import NUM_HEX_INDEXES_FOR_PATH, NUM_HEX_INDEXES_FOR_VERSION, NUM_HEX_INDEXES_FOR_STEP, NUM_HEX_INDEXES_FOR_VARIANT_VALUE

def get_position_strings_from_position_int(pos_int):
...
def convert_position_int_to_tile_variant_int(tile_int, variant_value=0):
    ...
    return int(path+version+step+hex(variant_value).lstrip('0x').zfill(3),16)
