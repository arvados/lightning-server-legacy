...
pipeline_template="grep.pipeline"
...
#
DATAUUID=`arv keep put --no-progress 1-input-pipeline-data`
echo "data uuid $DATAUUID"

# Register the pipeline with Arvados
#
template=` cat $pipeline_template | json -E "this.components.mattered.repository=\"$USER\"" `
...
#
echo "running: 'arv pipeline run --run-here --no-reuse --template $ARVUUID mattered::input=$DATAUUID'"
arv pipeline run --run-here --no-reuse --template $ARVUUID mattered::input=$DATAUUID
