// Extend a tile set given a base tile set.
//
// usage:
//
//  extendTileSetHG19 <choppedGffFileName> <fastjFileName> <chromosomeFastaFileName> <outputFastjFileName>
//
//
...
func (gss *GffScanState) advanceAndUpdateState( tileSet *tile.TileSet, chrFa []byte, startPos int, entryLen int, tagLen int ) {
  ...
  for ; (s + entryLen) >= (gss.nextTagStart + tagLen) ;  {
    ...
    if gss.refStart == gss.startPos[gss.startPosIndex-1] {
      ...
      if tagsMatch( gss.gffCurSeq, chrFa[ sprv : sprv+tagLen ], chrFa[ scur : scur+tagLen ] ) {
        handlePossibleNewTile( tileSet, gss.gffCurSeq, gss.notes )
      } else {
  ...
  //if gDebugFlag { fmt.Printf(">> EXTENDING (at end) %d to +%d (%d to %d inclusive)\n", gss.refgss.refEnd, e+1, gss.refEnd, e ) }


  dn := (s + entryLen) - (gss.refStart + gss.refLen)

  //gss.gffCurSeq = append( gss.gffCurSeq, chrFa[ gss.refStart + gss.refLen : s + entryLen ]... )
...
func handlePossibleNewTile( tileSet *tile.TileSet, tileSeq []byte, notes []string ) {

  n := len(tileSeq)
  if n < 48 { panic("tileSeq < 48!") }

  NormalizeTileSeq( tileSeq )

  stag := string(tileSeq[0:24])
  etag := string(tileSeq[n-24:n])
  body := string(tileSeq[24:n-24])

  baseTileId,ok := tileSet.TagToTileId[ stag ]
  if !ok {
  ...
  tileCopies := tileSet.TileCopyCollectionMap[ baseTileId ]

  newTileFlag := true

  for _,v := range tileCopies.Body {
    if v == body {

      if gDebugFlag { fmt.Printf(">>>dup\n>>>>%s\n", tileSeq) }
  ...
  if newTileFlag {
    ...
    newTileId := fmt.Sprintf("%s.%03x", baseTileId, len( tileCopies.Body ) )

    newCopyNum := len( tileCopies.Body ) + 1
    newN := len( tileSeq )
    ...
    header.Notes = append( header.Notes, notes... )

    newMeta,_ := json.Marshal( &header )

    tileSet.AddTile( newTileId, string(tileSeq), string(newMeta) )

    if gDebugFlag { fmt.Printf(">>> ***\n>>> *** adding tile [[ %s ]] %d --\n>%s\n%s\n\n", newTileId, newCopyNum, newMeta, tileSeq) }
...
func main() {
  ...
  if len(os.Args) != 5 {
    fmt.Println("usage:")
    fmt.Println("./extendTileSet.go <choppedGffFileName> <fastjFileName> <chromosomeFastaFileName> <outputFastjFileName>")
    os.Exit(0)
  }
  ...
  for scanner.Scan() {
    ...
    if typ == "REF" {
      ...
      gss.advanceAndUpdateState( tileSet, chrFa, s, entryLen, tagLen )
      if gss.startPosIndex == len(gss.startPos) { break }
  ...
  //outFastjFn := "/scratch/abram/tmp/test.fastj"

  if gDebugFlag { fmt.Println(">>>> FINAL FASTJ ASSEMBLY", outFastjFn) }

  tileSet.WriteFastjFile( outFastjFn )
