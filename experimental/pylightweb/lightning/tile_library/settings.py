...
LANTERN_TIMEOUT = 0.001
LANTERN_NAME_FORMAT_STRING = '^([0-9a-f]{%i}\.[0-9a-f]{%i}\.[0-9a-f]{%i})\.[0-9a-f]{%i}(?:\+([0-9a-f]+)$|$)' % (
...
######## Settings checks itself on initialization ########
######## Check format of CHR_PATH_LENGTHS ##############
# Should be the same length as the number of chromosomes plus 1
assert len(CHR_PATH_LENGTHS) == CHR_NONEXISTANT
for i, length in enumerate(CHR_PATH_LENGTHS):
    assert type(length) == int
    if i > 0:
        #Each item in the list should be greater than or equal to the item before it
        assert length >= CHR_PATH_LENGTHS[i-1]
    else:
        #The first item should always be 0
        assert length == 0
assert CHR_PATH_LENGTHS[-1] <= int('f'*NUM_HEX_INDEXES_FOR_PATH,16)

######## Check format of CYTOMAP ##################
assert len(CYTOMAP) == CHR_PATH_LENGTHS[-1]
for s in CYTOMAP:
    assert type(s) == str

######## Check format of Supported Statistics

supported_statistic_type_ints = [i for i, j in STATISTICS_TYPE_CHOICES]
supported_chr_ints = [i for i,j in CHR_CHOICES]
assert len(supported_statistic_type_ints) == len(CHR_CHOICES)+2
assert supported_statistic_type_ints == [GENOME] + supported_chr_ints + [PATH]
