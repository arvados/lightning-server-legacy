...
pipeline_template="customTask.pipeline"
...
#
DATAUUID=`arv keep put --no-progress 5-custom-task-data`
echo "data uuid $DATAUUID"

# Register the pipeline with Arvados
#
template=` cat $pipeline_template | json -E "this.components.denouement.repository=\"$USER\"" `
...
#
echo "arv pipeline run --run-here --no-reuse --template $ARVUUID denouement::MountUUID=$DATAUUID"
arv pipeline run --run-here --no-reuse --template $ARVUUID denouement::MountUUID=$DATAUUID
