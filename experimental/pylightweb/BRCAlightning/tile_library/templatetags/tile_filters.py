...
def get_SNP_INDEL_annotations(tile_variant):
    ...
    try:
        return tile_variant.genome_variants.all()
    except ObjectDoesNotExist:
        raise BaseException("TileVariant does not exist")

@register.filter
def get_readable_annotation_text(annotation):
    assert type(annotation)==GenomeVariant, "Expects type GenomeVariant for input"
    try:
        ref_bases = annotation.reference_bases
        alternate_bases = annotation.reference_bases
