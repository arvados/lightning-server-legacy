...
genomes_and_chromosomes = [i for i,j in settings.STATISTICS_TYPE_CHOICES]
genomes_and_chromosomes.remove(settings.PATH)
...
def initialize(silent=False, quiet=True):
    impossible_name, impossible_varname = functions.get_min_position_and_tile_variant_from_chromosome_int(settings.PATH)
    ...
    for path in range(settings.CHR_PATH_LENGTHS[-1]):
...
def update(path_only=False, silent=False, quiet=True):
    ...
    try:
        ...
        for path in range(settings.CHR_PATH_LENGTHS[-1]):
            ...
            if not silent and not quiet:
                print tile_info, tile_var_info
            s = GenomeStatistic.objects.filter(statistics_type=settings.PATH).get(path_name=path)
            s.num_of_positions = tile_info['num_of_positions']
            s.num_of_tiles = tile_var_info['num_of_tiles']
            s.max_num_positions_spanned = tile_var_info['max_pos_spanned']
            s.save()
